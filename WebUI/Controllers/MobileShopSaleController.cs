    ...
    public class MobileShopSaleController : BaseController
    {
        ...
        public ActionResult Index(string dateFrom, string dateTo, string customerId)
        {
            try
            {
                ...
                {
                    return View(AutoMapperConfiguration.mapper.Map<IEnumerable<MobileShopSaleViewModel>>(_saleService.GetAll(identity.CompanyId, identity.BranchId, dfrom.Value, dto.Value, customerId)));
                }
                if (!string.IsNullOrEmpty(dateFrom) && !string.IsNullOrEmpty(dateTo))
                {
                    return View(AutoMapperConfiguration.mapper.Map<IEnumerable<MobileShopSaleViewModel>>(_saleService.GetAll(identity.CompanyId, identity.BranchId, dfrom.Value, dto.Value)));
                }
                if (!string.IsNullOrEmpty(customerId))
                {
                    return View(AutoMapperConfiguration.mapper.Map<IEnumerable<MobileShopSaleViewModel>>(_saleService.GetAllByCustomer(customerId)));
                }
                return View();
            }
            catch (Exception ex)
        ...
        public ActionResult Edit(string id)
        {
            try
            {
                ...
                ViewBag.ProductList = serializer.Serialize(_rawSqlService.GetBranchwiseProductStockGreaterThanZero(identity.CompanyId, identity.BranchId));
                ViewBag.PromotionalDiscountList = serializer.Serialize(_rawSqlService.GetPromotionalPointAndDiscount(DateTime.Today));
                MobileShopSaleViewModel salevm = AutoMapperConfiguration.mapper.Map<MobileShopSaleViewModel>(_saleService.GetById(id));
                List<MobileShopSaleDetailViewModel> saleItems = AutoMapperConfiguration.mapper.Map<List<MobileShopSaleDetailViewModel>>(_saleService.GetAllSaleDetailbyMasterId(id).ToList());
        ...
        public ActionResult ReportSaleSummary(string dateFrom, string dateTo, string customerId)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(dateFrom))
                {
                    sales = AutoMapperConfiguration.mapper.Map<IEnumerable<MobileShopSaleViewModel>>(_rawSqlService.GetAllSalesSummary(identity.CompanyId, identity.BranchId, dateFrom, dateTo, customerId));
                }
                return View(sales);
            }
            catch (Exception ex)
        ...
        public ActionResult ReportSaleClosing(string dateFrom, string dateTo)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(dateFrom))
                {
                    sales = AutoMapperConfiguration.mapper.Map<IEnumerable<MobileShopSaleViewModel>>(_rawSqlService.GetAllSaleClosing(identity.CompanyId, identity.BranchId, dateFrom, dateTo, identity.Name));
                }
                return View(sales);
            }
            catch (Exception ex)
